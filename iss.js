...
const fetchCoordsByIP = function(ip, callback) {
  ...
  request(url, (error, response, body) => {
    ...
    const {latitude, longitude } = data; // n?eed some explanation for this part.
...
const fetchISSFlyOverTimes = function(coords, callback) {
  ...
  request(url, (error, response, body) => {
    ...
    const data = JSON.parse(body); // does the order of this matter?
...
const nextISSTimesForMyLocation = function(callback) {
  fetchMyIP((error, ip) => {
    if (error) {
      return callback(error, null); // why return here????
    }
